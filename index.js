...
class LeaderBoard {
  ...
  bottom(num_players){
    let topList=Object.keys(this.players).map(key=>
      [Number(key), this.players[key].average()]).sort((a,b)=>a[1]-b[1]).map(element=>element[0]).slice(0,num_players)
...
console.log('Top 3 [' + leader_board.bottom(3) + '] should equal [1, 2, 3]:')
console.log(array_equals(leader_board.bottom(3), [1, 2, 3]))
...
console.log('Top 2 [' + leader_board.bottom(2) + '] should equal [1, 2]:')
console.log(array_equals(leader_board.bottom(2), [1, 2]))
...
console.log(array_equals(leader_board.bottom(3), [3, 1, 2]))
